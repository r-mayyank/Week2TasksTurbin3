import { getBase58Decoder, getBase64Encoder, assertIsAddress, assertIsSignature, getTransactionDecoder, pipe, getBase64EncodedWireTransaction, compileTransaction, partiallySignTransactionMessageWithSigners, sendAndConfirmTransactionFactory, setTransactionMessageLifetimeUsingBlockhash, assertIsTransactionMessageWithBlockhashLifetime, signTransactionMessageWithSigners, getSignatureFromTransaction, createSolanaRpc, createSolanaRpcSubscriptions, createTransactionMessage, isTransactionSigner, setTransactionMessageFeePayerSigner, setTransactionMessageFeePayer, appendTransactionMessageInstruction, appendTransactionMessageInstructions, SolanaError, SOLANA_ERROR__TRANSACTION_ERROR__UNKNOWN, isSolanaError, SOLANA_ERROR__INSTRUCTION_ERROR__GENERIC_ERROR, AccountRole, createKeyPairFromBytes, getBase58Encoder, createSignerFromKeyPair, getComputeUnitEstimateForTransactionMessageFactory, createNoopSigner, assertIsTransactionSigner, getPublicKeyFromAddress, verifySignature, isInstructionForProgram, isInstructionWithData } from '@solana/kit';
...
// src/core/send-and-confirm-transaction-with-signers.ts
...
function createTransaction({
  ...
  return pipe(
    ...
    (() => {
      let selectedVersion;
      if (version === void 0 || version === "auto") {
        selectedVersion = instructions.some(
          (ix) => "addressTableLookup" in ix && ix.addressTableLookup != null || "addressTableLookups" in ix && Array.isArray(ix.addressTableLookups) && ix.addressTableLookups.length > 0
        ) ? 0 : "legacy";
      } else {
...
function getSignatureFromBytes(sigBytes) {
...
function lamportsToSol(lamports, decimals = 9) {
  ...
  const solValue = Number(lamports) / 1e9;
  return new Intl.NumberFormat("en-US", {
    maximumFractionDigits: maxDecimals,
    minimumFractionDigits: 0
  }).format(solValue);
}
async function verifySignatureForAddress(address, signature, message) {
