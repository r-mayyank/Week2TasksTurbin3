...
function base64BytesToAddress(base64Bytes) {
  const maybeAddress = kit.getBase58Decoder().decode(kit.getBase64Encoder().encode(base64Bytes));
  kit.assertIsAddress(maybeAddress);
  return maybeAddress;
}
function base64BytesToSignature(base64Bytes) {
  const maybeSignature = kit.getBase58Decoder().decode(kit.getBase64Encoder().encode(base64Bytes));
  kit.assertIsSignature(maybeSignature);
  return maybeSignature;
}
function base64BytesToString(base64Bytes) {
  return new TextDecoder().decode(kit.getBase64Encoder().encode(base64Bytes));
}
  ...
  return async function sendAndConfirmTransactionWithSigners(transaction, config = { commitment: "confirmed" }) {
    if ("messageBytes" in transaction == false) {
      if ("lifetimeConstraint" in transaction === false) {
        ...
        transaction = kit.setTransactionMessageLifetimeUsingBlockhash(latestBlockhash, transaction);
        kit.assertIsTransactionMessageWithBlockhashLifetime(transaction);
      }
...
function createTransaction({
  ...
  return kit.pipe(
    ...
    (() => {
      ...
      return kit.createTransactionMessage({ version: selectedVersion });
    })(),
    (tx) => {
...
function getSignatureFromBytes(sigBytes) {
  return kit.getBase58Decoder().decode(sigBytes);
}
...
async function prepareTransaction(config) {
  ...
  if (config.blockhashReset || "lifetimeConstraint" in config.transaction == false) {
    ...
    if ("lifetimeConstraint" in config.transaction == false) {
      ...
      config.transaction = kit.setTransactionMessageLifetimeUsingBlockhash(
        latestBlockhash,
        config.transaction
      );
...
async function verifySignatureForAddress(address, signature, message) {
  const publicKey = await kit.getPublicKeyFromAddress(address);
  if (typeof message === "string") {
    message = new TextEncoder().encode(message);
  }
  if (typeof signature === "string") {
    signature = kit.getBase58Encoder().encode(signature);
  }
  return kit.verifySignature(publicKey, signature, message);
}
...
exports.base64BytesToAddress = base64BytesToAddress;
exports.base64BytesToSignature = base64BytesToSignature;
exports.base64BytesToString = base64BytesToString;
...
exports.getSignatureFromBytes = getSignatureFromBytes;
...
exports.verifySignatureForAddress = verifySignatureForAddress;
